            ...
            <Tabs defaultValue="today" className="w-full">
              ...
              <TabsContent value="tomorrow" className="space-y-4">
                {(() => {
                  ...
                  const tomorrowMenuData = (weeklyMenuData as any[])?.find(m => m.date === tomorrowDate);
                    ...
                    <Card>
                      ...
                      <CardContent>
                        {tomorrowMenu ? (
                          <div className="space-y-0">
                            <MealSection 
                              ...
                              onEdit={() => {
                                if (tomorrowMenuData) {
                                  setEditingMenu({
                                    id: tomorrowMenuData.id,
                                    date: tomorrowDate,
                                    mealType: 'breakfast',
                                    items: tomorrowMenu.breakfast ? tomorrowMenu.breakfast.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', tomorrowMenu.breakfast || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                              ...
                              onEdit={() => {
                                if (tomorrowMenuData) {
                                  setEditingMenu({
                                    ...
                                    date: tomorrowDate,
                                    mealType: 'lunch',
                                    items: tomorrowMenu.lunch ? tomorrowMenu.lunch.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', tomorrowMenu.lunch || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                            <MealSection 
                              title="Evening Snacks" 
                              items={tomorrowMenu.eveningSnacks}
                              isAdmin={canEditMenu}
                              onEdit={() => {
                                if (tomorrowMenuData) {
                                  setEditingMenu({
                                    ...
                                    date: tomorrowDate,
                                    mealType: 'eveningSnacks',
                                    items: tomorrowMenu.eveningSnacks ? tomorrowMenu.eveningSnacks.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', tomorrowMenu.eveningSnacks || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                              ...
                              onEdit={() => {
                                if (tomorrowMenuData) {
                                  setEditingMenu({
                                    ...
                                    date: tomorrowDate,
                                    mealType: 'dinner',
                                    items: tomorrowMenu.dinner ? tomorrowMenu.dinner.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', tomorrowMenu.dinner || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                          ...
                          <div className="text-center py-8 text-muted-foreground">
                            No menu available for {new Date(tomorrowDate + 'T00:00:00').toLocaleDateString()}
                          </div>
                        )}
                      </CardContent>
                    </Card>
              ...
              <TabsContent value="dayafter" className="space-y-4">
                {(() => {
                  ...
                  const dayAfterMenuData = (weeklyMenuData as any[])?.find(m => m.date === dayAfterDate);
                      ...
                      <CardContent>
                        {dayAfterMenu ? (
                          <div className="space-y-0">
                            <MealSection 
                              title="Breakfast" 
                              items={dayAfterMenu.breakfast}
                              isAdmin={canEditMenu}
                              onEdit={() => {
                                if (dayAfterMenuData) {
                                  setEditingMenu({
                                    id: dayAfterMenuData.id,
                                    date: dayAfterDate,
                                    mealType: 'breakfast',
                                    items: dayAfterMenu.breakfast ? dayAfterMenu.breakfast.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', dayAfterMenu.breakfast || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                              ...
                              onEdit={() => {
                                if (dayAfterMenuData) {
                                  setEditingMenu({
                                    ...
                                    date: dayAfterDate,
                                    mealType: 'lunch',
                                    items: dayAfterMenu.lunch ? dayAfterMenu.lunch.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', dayAfterMenu.lunch || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                            <MealSection 
                              title="Evening Snacks" 
                              items={dayAfterMenu.eveningSnacks}
                              isAdmin={canEditMenu}
                              onEdit={() => {
                                if (dayAfterMenuData) {
                                  setEditingMenu({
                                    ...
                                    date: dayAfterDate,
                                    mealType: 'eveningSnacks',
                                    items: dayAfterMenu.eveningSnacks ? dayAfterMenu.eveningSnacks.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', dayAfterMenu.eveningSnacks || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                              ...
                              onEdit={() => {
                                if (dayAfterMenuData) {
                                  setEditingMenu({
                                    ...
                                    date: dayAfterDate,
                                    mealType: 'dinner',
                                    items: dayAfterMenu.dinner ? dayAfterMenu.dinner.split(',').map((item: string) => item.trim()) : []
                                  });
                                  menuEditForm.setValue('items', dayAfterMenu.dinner || '');
                                  setShowEditDialog(true);
                                }
                              }}
                            />
                          ...
                          <div className="text-center py-8 text-muted-foreground">
                            No menu available for {new Date(dayAfterDate + 'T00:00:00').toLocaleDateString()}
                          </div>
                        )}
                      </CardContent>
                    </Card>
                              ...
                              <MealSection 
                                ...
                                onEdit={() => {
                                  if (dayMenuData) {
                                    setEditingMenu({
                                      id: dayMenuData.id,
                                      date: dayMenu.dateStr,
                                      mealType: 'breakfast',
                                      items: dayMenu.breakfast ? dayMenu.breakfast.split(',').map((item: string) => item.trim()) : []
                                    });
                                    menuEditForm.setValue('items', dayMenu.breakfast || '');
                                    setShowEditDialog(true);
                                  }
                                }}
                              />
                                ...
                                onEdit={() => {
                                  if (dayMenuData) {
                                    setEditingMenu({
                                      ...
                                      date: dayMenu.dateStr,
                                      mealType: 'lunch',
                                      items: dayMenu.lunch ? dayMenu.lunch.split(',').map((item: string) => item.trim()) : []
                                    });
                                    menuEditForm.setValue('items', dayMenu.lunch || '');
                                    setShowEditDialog(true);
                                  }
                                }}
                              />
                                ...
                                onEdit={() => {
                                  if (dayMenuData) {
                                    setEditingMenu({
                                      ...
                                      date: dayMenu.dateStr,
                                      mealType: 'eveningSnacks',
                                      items: dayMenu.eveningSnacks ? dayMenu.eveningSnacks.split(',').map((item: string) => item.trim()) : []
                                    });
                                    menuEditForm.setValue('items', dayMenu.eveningSnacks || '');
                                    setShowEditDialog(true);
                                  }
                                }}
                              />
                                ...
                                onEdit={() => {
                                  if (dayMenuData) {
                                    setEditingMenu({
                                      ...
                                      date: dayMenu.dateStr,
                                      mealType: 'dinner',
                                      items: dayMenu.dinner ? dayMenu.dinner.split(',').map((item: string) => item.trim()) : []
                                    });
                                    menuEditForm.setValue('items', dayMenu.dinner || '');
                                    setShowEditDialog(true);
                                  }
                                }}
                              />
